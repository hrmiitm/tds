...
- **Login shell** → profile files (`.bash_profile`, `.zprofile`)  
- **Interactive shell** → rc files (`.bashrc`, `.zshrc`)  
- **Non-interactive shell** → no startup files (or `$BASH_ENV`)  
...
│   ├─ YES
│   │   ├─ Read /etc/profile
│   │   ├─ ~/.bash_profile (if exists) → STOP
│   │   ├─ ~/.bash_login   (else if exists) → STOP
│   │   └─ ~/.profile      (fallback) → STOP
│   └─ NO
│
...
│   └─ NO
│
└─ Is this NON-INTERACTIVE (script)?
    ├─ YES
    │   ├─ $BASH_ENV set?
    │   │   ├─ YES → Read file in $BASH_ENV
    │   │   └─ NO  → Read nothing
    └─ END
```
